...
\noindent {\bf Q1: Architecture}  You can find a description of the model in the second half of the slides from lecture \#6.
\begin{itemize}
...
\vspace{2em}
\noindent {\bf Q3: Implementing the Language Model} In \texttt{model.py}, you will find the main model class FeedForwardLanguageModel, with its initialization method written.  Implement the \texttt{.forward} method, using its docstring as a guide.  [Hint: \texttt{ops.concat}, which we provide, will be necessary.  As above, do not provide any ``direct connections''.]
