...
\noindent {\bf Q1: Worked example}  Consider the function $f(x) = x^2 \times 3x$.
\begin{itemize}
  ...
  \item For $x = 2$:
    ...
    \end{itemize}
  \item Consider the node corresponding to $x^2$ in the graph.  For each of the following, write a symbolic expression and the numerical value (at $x=2$) for:
    \begin{itemize}
...
\begin{itemize}
  ...
  \item Implement \texttt{SGNS.forward}.  This represents one ``forward pass'' of the skip-gram with negative sampling model, i.e. this computes $P(1 | w, c)$ for a batch of inputs.  You should use dot\_product\_rows here.
